...
- **署名解析出来了，但没上线。** 第二批每篇每档都带一行
  「By Associated Press, adapted by Newsela staff」。它必须从正文里摘出去
  （不摘就变成 b1，会被当成文章第一句引给学生），`parse.py` 摘了，
  但没往 `articles.json` 里放：要显示它得给 `reading` 那张表加一列、
  改 DTO、改阅读室，而那张表 pro 也在用。**要不要显示作者，是一个内容授权
  问题，等 owner 定。** 定了之后数据是现成的。
