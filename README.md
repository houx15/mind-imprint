...
1. **项目部分调试** — 工作室 / 写作旅程（立项 → 提案 → 证据地图 → 正文 → 定稿 → 回顾）的 bug 修复与体验打磨。
2. **过程评估：策略设计 + 开发** — 在已定稿的双轴模型上设计并实现评估逻辑。数据地基见 [评估数据落库指南](docs/2026-08-11-evaluation-data-storage-guide.md)，代码见 [评估与教师端代码地图](docs/2026-08-11-evaluation-and-teacher-code-map.md)。
3. **教师端开发** — 继续完善教师 / 管理员端（代码同见上方代码地图）。
4. **课程部分重构** — 重构 Course（引导式单课）面。

> 更远（非本轮）：计费 / 权益方案设计（填 `HasEntitlement` 接缝）· 评估异步化（river worker）为规模化预留 · 评估语义扩展、a11y / 整洁项。
