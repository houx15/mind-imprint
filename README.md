...
| 房间 | 是什么 |
|---|---|
| **项目管理** | 看板 + 甘特图 + 活动日志，把长研究拆成任务、进出各房间。 |
| **阅读** | 两个视图：**图书馆**（收藏的文献，带标签与 待读/在读/读完 状态）+ **探索**（下述**兔子洞地图**）。 |
| **写作** | 一个**朴素的写作面**——学生**自己写**正文（textarea + Markdown 预览 + 自动存草稿），也可粘贴/上传。AI 只陪想、查论证与结构（**绝不代写正文**），一次一问。回顾处**完成 → 过程评估**，成品可**导出**（.docx / .xlsx）带走。 |
| **回顾** | 过程回看 + 触发过程评估（「你的思维印记」）。 |
