...
**铁律的适用边界（别扩大化）：** 「AI 不替学生做 / 学生确认」这条**只针对学生的写作（正文/body text）**——AI 绝不代写正文。**它不适用于确定性的系统步骤**：生成计划、从学生已陈述的研究问题或标准框架模板派生大纲/提纲，都是系统该自动做的事，**不是「操纵」、不需要为「防操纵」加学生确认门槛**。不要用铁律②去质疑这些设计。
...
- **每份实施 plan 写完后，必须先对照 `docs/2026-08-09-all-statuses.md` 的相关章节逐条校验一致性，再进入实现。** 该文档是写作全流程（各状态的页面/卡片/AI 角色/状态转移）行为的单一真相源；plan 与它冲突时，以它为准（或显式回到用户确认）。同理，架构 spec 也以它为行为真相源。
