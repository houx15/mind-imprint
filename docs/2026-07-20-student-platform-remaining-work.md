...
|---|---|---|---|
| **N1 · Close the loop** | Project-creation entry point + finish-button discoverability + the cheap correctness seams that make the existing loop trustworthy (live gate counts, onboarding producer↔reader, `GetCardInstance` scoping, event-model normalization) | S–M | Makes the product **reachable and reliable end-to-end**. A student cannot start a project today. Highest leverage. **← planning first.** |
