...
role='student'; stage='snippets';
await page.goto('http://127.0.0.1:5234/writings/w1'); await page.waitForTimeout(1100);
const preview=page.getByRole('button',{name:'放大查看这一篇的结构'});
await preview.screenshot({path:'/tmp/ui0922-minimap.png'});
if(await preview.locator('svg g').count() !== outline.length) throw Error('Missing nodes');
await preview.click();
await page.getByRole('dialog').waitFor({state:'visible'});
if(await page.getByRole('button',{name:'关闭结构预览'}).evaluate(el=>el!==document.activeElement)) throw Error('Focus not moved');
await page.keyboard.press('Tab');
if(!await page.evaluate(()=>document.querySelector('dialog').contains(document.activeElement))) throw Error('Focus escaped');
await page.keyboard.press('Escape');
await page.getByRole('dialog').waitFor({state:'detached'});
if(!await preview.evaluate(el=>el===document.activeElement)) throw Error('Focus not restored');
await page.screenshot({path:'/tmp/ui0922-minimap-page.png'});
console.log('MiniMap: all nodes, dialog keyboard focus, Escape and restored focus passed');
