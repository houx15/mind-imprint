const {chromium}=require(require('path').resolve('apps/lite-web/node_modules/@playwright/test'));
const fs=require('fs');
...
let role='teacher',stage='outline';
...
else if(p.endsWith('/writings/w1/draft'))data={body:essay,updatedAt:date};
...
const shots=[['assignments','/assignments'],['assignment-detail','/assignments/a1'],['assignment-grading','/assignments/a1?tab=grading'],['grading','/gradings/g1'],['parent-reports','/parent-reports'],['parent-report-editor','/parent-reports/p1']];
for(const [name,path]of shots){await page.goto('http://127.0.0.1:5234'+path);await page.waitForTimeout(1000);if(await page.locator('vite-error-overlay').count())throw new Error('Vite build overlay');await page.screenshot({path:`${OUT}/${name}.png`});console.log(name,(await page.locator('body').innerText()).slice(0,220));}
role='student';for(const s of ['outline','flow','snippets','draft']){stage=s;await page.goto('http://127.0.0.1:5234/writings/w1');await page.waitForTimeout(1100);await page.screenshot({path:`${OUT}/writing-${s}.png`});console.log('writing-'+s,(await page.locator('body').innerText()).slice(0,180));}
