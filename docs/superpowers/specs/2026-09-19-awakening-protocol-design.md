...
## 9 · 上线之后走查出来的东西（2026-09-19）

设计定稿之后先上线，再用两条走查各走一遍。两条分工不同，而且**各自抓到了对方
看不见的东西**：

| | 抓到的 |
|---|---|
| `apps/lite-web/e2e/awakening-walk.mjs`（接口） | 报告字段名全错、节点推进两处各算各的、nil 切片变 null |
| `apps/lite-web/e2e/awakening-eyeball.spec.ts`（真浏览器） | 最后一问被问了两遍、入口没跟着变、词下面那句话在她面前谈论她 |

接口走查看不见「别扭」，浏览器走查看不见字段名。两条都要。

**五个场景**（接口那条）：新学生走长路 / 中途退出再回来 / 说不清的学生 /
老学生再走一趟 / 观察者选择不继续。每个场景自己注册一个账号 —— 共用一个种子
账号的话，「她还没走过一趟」这个前提就由跑的顺序在守，而那守不住。

线上跑法写在两个文件的头部注释里。🚨 线上要用轻量版体验班的 join code，
Demo School 是 pro，拿它注册出来的账号打轻量版的路一律 404。

### 几条值得记住的

- **一个缺失的 json 标签在任何地方都看不见** —— 树上的词是对的、日志没有报错、
  Go 测试全绿，只有把结构真的序列化一次再按前端读的名字取才看得出来。
- **Go 的 nil 切片 marshal 成 `null`**，而前端对它调 `.join()`。服务端改成空
  切片之后，**库里已经写下的行改不回去**，所以客户端那一层也必须挡。
- **同一个问题有两处答案，迟早会分道扬镳。** 「下一个节点是谁」原来算两遍，
  在一个薄答案落在最后一个节点时两边不一致，回 400。
- **判据要盯住真失败，不是它的影子。** 幻引第一版判所有成对引号，把一条模型
  给自己说法起名字的好回复判死了。改成只判被归给她的那些；第二次仍然不逐字时
  **去掉引号**而不是丢掉整轮 —— 不变量保住，她不会看见一个死掉的终端。

### 第二轮走查（同日，重跑时发现的）

第一轮那两条走查本身有毛病，重跑时才暴露出来：

- 🚨 **走查其实一直打在本地。** `-c e2e/online.config.ts` 说的是线上，而
  `freshAccount` 读的是 env（默认 `localhost:5174`），它看不见那份 config 的
  `baseURL`。九条用 freshAccount 的 spec 全都静默打了本地 dev server。
  被发现纯属运气 —— 本地那台换成了旧代码，走查红在第一屏「找不到开始觉醒
  协议」，读起来完全像入口没做出来。**一条打错 host 的走查，绿的时候什么都不
  证明，红的时候还会诬告产品。** 现在三个值一起在 config 里定死。
- 🚨 **最核心的那条判据一直被跳过。** 「又出现的词强度要涨」只在有 confirm
  时才验，而场景 4 的她故意换了方向，于是一次都没验过。补了场景 6（方向没变
  的重做）：线上实测「海洋」两趟都长出来，强度 2，树上挂着两趟各一条来源。
- 🚨 **报告下半部分从来没被人眼看过。** 房间是 fixed 浮层、正文在内层滚动
  容器里，`fullPage` 截出来的高度正好等于视口。按元素整块截也不行 —— 得到的
  是一张下半部分整片空白的长图。改成把内层容器滚到底再截第二张。
- 🚨 **我们把自己的故障写成了一句关于她的判断。** 选词那次回了写坏的 JSON，
  零词，报告于是对一个写了八段具体经历的学生说「你写下的内容里还没有足够具体
  的原话可以作为根据」。同一份文本在另一个账号上长出了两个词。
  改：选词读不懂就再问一次（这是**终点**调用，报告只生成一次，没有第二次
  机会），并用 `Report.SelectionFailed` 把「她确实没有可落的词」和「我们没跑
  成」分开，界面对这两件事说不一样的话。

## 10 · 资产那条边界

这些文件放在营销站那个公开桶（`mind-open` / `mind-assets.uni-robot.cn`）的
`awakening/v1/` 下，而不是产品自己的 OSS。

那条「站点资产和学生端分开」的边界仍然成立，因为分的是**数据**：学生上传的
东西走产品那套私有桶 + 签名下载。这 21 个文件是**产品美术** —— 它要被缓存、
要永远可读，而一张背景图的链接过期没有任何意义。

上传：`SITE_ASSET_DIR=<含 awakening/v1/ 的目录> deploy/upload-site-assets.sh`。
