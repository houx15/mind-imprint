...
- **chat/course 作用域**报告 → 只渲染**通用内核**（总览-简/双轴读数/交互证据/提示词透镜/下一步）；无官方投影、无作品与过程；证据地图退化为无 official 节点（或整段不渲染，实现期定，**默认：core 报告不渲染证据地图**，因其核心价值在项目证据串联）。
...
TeacherReportView **复用 Spec B 学生报告的段组件**（`apps/web/src/shell/report/DualAxisReport.tsx` 的 D/A 卡、官方投影、提示词透镜、交互证据、作品与过程渲染）。抽取共享段组件（若当前耦合在 `DualAxisReport` 内，实现期做**最小抽取**，学生端渲染不变——回归测试保证）。教师专属外壳（目录导航、面包屑、导出入口、教师文案 register）为 TeacherReportView 独有。
