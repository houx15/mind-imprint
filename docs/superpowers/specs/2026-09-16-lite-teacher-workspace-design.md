...
它**不是**第 6 节否掉的那个通用数字检测器。判据是**数字 + 人的量词**这一个形状：数字（阿拉伯数字或中文数字）后面紧跟 `人` / `位` / `名`，或者 `个` 后面把名词写出来（`个学生` / `个同学` / `个孩子` / `个人`）。年份、难度档、字数、日期、第几段都带数字，一个都不落进这个形状里。证据侧是本轮工具数出来的人数、班级人数（系统 prompt 第一句就把它交给了模型）、以及老师自己说过的人数。

塑造这条判据的三个误判，都在单元测试里钉着：
