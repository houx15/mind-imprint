...
- **Live-verify:** send → CRAAP proposal → open → fill → submit → fill persists + coach refeed reacts. **Re-scoped (whole-branch-review): the evidence-node mint + `every_source_evaluated` gate flip are DEFERRED to Slice 6** — CRAAP's completion is anchor-based, but the schema field renderer wired here only produces `field_values`, never `anchors`; the material-annotation surface that produces anchors is Slice 6. The mint transport itself is verified given a satisfying hand-built anchor set (`TestProjectCardSubmit`), gated behind `CompleteCard`'s `complete==true` so a non-satisfying (today: every real) submit correctly stays `active`, not falsely `completed`. ✓
