...
- **Change during execution (Task 3, fix round 1):** the `strong_engagement` praise rule was **removed entirely** — it was unreachable behind `stuck_no_output` (same report-less-with-turns population, watch wins), and praising usage/turn-count violates 铁律② (不操纵). Praise is now finished-thinking only (`first_report`/`produced_report`); `praiseCard` takes only `s StudentWeek`, and `Detect` no longer computes class turn stats. The Task 3 code blocks above still show the original `strong_engagement` form — the committed code is authoritative; the spec was updated to match the removal.
