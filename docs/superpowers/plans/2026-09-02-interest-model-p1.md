...
## 执行中改掉的两处（计划与实现不一致时，以下面为准）

**1 · Task 8 从「三个完成点各挂一个钩子」改成「打开树时惰性补采」。**

计划里写的是在 `finishReading` / `finishWritingAtom` / 项目转复盘三处各加一行
调用。实现时否掉了，两个理由：

- `finishReading` 现在是一次**瞬时翻转**（它自己的注释原话：a plain synchronous
  flip），没有任何模型调用。在上面挂一次三秒的采集，会把一个「点一下就好」的
  动作变成一次等待——而她点完之后要看的报告本身还要再等一次生成。
- **这个代码库里没有 fire-and-forget 的先例。** `internal/api` 里全部 `go func()`
  不是 SSE 心跳，就是同一个请求里 `WaitGroup` 汇合的并行取数；没有一处是
  「handler 返回之后还在后台跑」。为一个新功能开这个先例，代价是无人观测的
  goroutine 和一类只在生产上出现的 bug。

现在的做法照抄 `atom_report` 的惰性生成：`GET /interest/tree` 里先补采最近三个
「已完成、还没采过」的 atom（并行，所以墙钟约等于一次调用），再读树。等待付在
她正盯着那棵树等它长的时候——那是这次等待唯一说得通的地方。

⏭ **river 的表已经迁移过了，但客户端和 worker 都还没有（P4 才建）。** river 起来
之后，这一整块应该变成一个在「完成」那一刻入队的任务，惰性这层就可以删掉。

**2 · 迁移 0116 多了一列：`atom.interest_harvested_at`。**

惰性补采需要知道哪些 atom 采过了。语义是**「尝试过一次，无论结果如何」**，不是
「长出过词」——一篇很薄的阅读完全可能一个词都采不出来，而那个结果在
`interest_keyword` 里和「从没采过」长得一模一样；按「有没有长出词」判断，就会
对同一篇薄阅读每次打开树都重发一次旗舰调用。这条教训直接来自 `0104` 的
`reading.questions_at`。所以**盖章发生在模型调用之前**。

0116 从未离开这个分支（只在一次性 testcontainer 里跑过），所以这一列写进 0116
本身，而不是另开一个只加一列的 0117。

## P1 完成状态

八个任务全部完成，48 个测试绿：
- `internal/disciplines` 13（学科表结构、别名全局唯一、asks 必须是问句、副本不漂移）
- `internal/interest` 25（三档路由边界、两种解析的全部丢弃规则）
- `internal/api` 10（迁移 0116 真的建得起来、树的形状、采集队列的两类花钱的错）
