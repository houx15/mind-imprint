...
**Deliberate deviation from the spec, needs a §16 amendment.** The spec has `pbl_project` as a root table with its own `pbl_thread_item`. This plan makes a project an **atom**, so it inherits ownership, activity, heartbeat, `atom_message` and `llm_call.atom_id` metering rather than growing a second substrate beside an identical one. `pbl_thread_item` is likely unnecessary as a result — S2 should check whether `atom_message` plus a payload column (the precedent is migration 0106's `coach_card_payload`) carries thread items instead.
