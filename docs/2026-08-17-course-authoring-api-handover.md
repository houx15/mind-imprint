...
**Date:** 2026-08-17
**Pinned at tag:** `course-authoring-v1.5.0` (annotated tag on `main`) — the stable, named snapshot of the course packages + authoring API to build against. It sits on the revision live on production and includes the G2/G7 changes below. (The tag, not a raw SHA or `main`, is what you pin.)
...
All three are **workspace packages with version `0.0.0`** — they are not independently semver-published. The *git tag is the version*: pin to **`course-authoring-v1.5.0`** (see "How to consume the pin" below).
...
**How to consume the pin.** These packages are `workspace:*` and are not published to any registry. Pin to the **annotated git tag `course-authoring-v1.5.0`** (on `main`) rather than a moving branch or a raw SHA — it is the named contract snapshot. Two ways to vendor:
- **Git submodule / subtree / sparse checkout** of `packages/course-contract`, `packages/course-runtime`, `packages/course-renderer` at tag `course-authoring-v1.5.0`. They are self-contained (only external deps are `zod`, `react-markdown`, `remark-gfm`, `react`).
...
- **`course-authoring-v1.5.0`** — renderer-visual + authoring-guidance (no contract change; every `CourseDefinition` valid under v1.4.0 is unchanged). Three things: **(1) every slot now centers its content vertically** — a short reading card, a single figure, or the two stacked blocks of a split column all float in the middle of their region instead of clinging to the top (media that fills its slot is unaffected). **(2) Images are click-to-enlarge**: clicking any course figure opens a lightbox where the image can be **zoomed with the scroll wheel** and **dragged to pan**; Escape, the ✕, or a backdrop click closes it. No authoring change — any `images` block gets this for free. **(3) §6.2 now recommends a stricter default layout discipline for generators** (prefer `full` / a 1:1 `split-horizontal` / `grid`; keep splits 1:1 unless a specific need justifies a weight; express "one column split into two" as two stacked blocks in a split slot). The full preset + ratio set stays valid in the contract — this is guidance, not a restriction. See §6.2.
...
**Recommended default discipline for generators (v1.5.0).** The full preset + ratio set above stays valid — this is *guidance*, not a contract restriction, because real courses have many legitimate layout situations. But a generator produces cleaner, more predictable slices if it **defaults to a small, strict palette and only reaches past it for a concrete reason**:
- **Prefer three shapes:** `full` (1 block), a **1:1 `split-horizontal`** (2 blocks side by side), and `grid` (3–4 small parallel blocks). These cover the large majority of slices.
- **Keep splits `1:1` by default.** Reach for a weighted ratio (`2:1`, `1:2`, `3:1`, …) only when one side genuinely carries the bulk — a wide side for video, a `1:2` for text-beside-PDF. Don't weight a split "just because."
- **Express "one column split into two vertical things" by stacking two blocks in a split slot**, not with a top-level `split-vertical`. A slot stacks its `blockIds` vertically (12px gap) and centers the pair — so a 1:1 `split-horizontal` with one block on the left and two stacked on the right *is* the "left single, right split in two" layout (three blocks, cleanly). Use a top-level `split-vertical` (top/bottom) sparingly — only when the whole slice is a single vertical relationship (a prompt directly above the one source it's about) with no horizontal structure.
- **Symmetric 4-block → `grid` (2×2).** Its two rows are equal height (true 1:1 halves per column), which a pair of stacked blocks can't guarantee.

Net: the generator's *default* vocabulary is `full` / 1:1 `split-horizontal` (optionally with a stacked column) / `grid`; the weighted ratios and `split-vertical` remain available for the specific cases that need them.
