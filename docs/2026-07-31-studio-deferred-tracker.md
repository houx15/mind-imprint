...
- [x] **J · #18 cards in forming + library (Both)** — reusable `CoachCardPanel` (proposal chip + 工具卡 self-summon deck + StudioCardSheet + persist/dismiss) mounted in forming (PlanBlock) + library (ReadingBlock FloatingCoach). Server: `coachProposeSurfaces` extended to forming/proposal_review/find_sources; the card-offer is mutually exclusive with the #13 dim-chip per turn (dim wins on 立题). Deck = the 3 persistable thinking cards (fact-opinion-value / certainty-spectrum / steelman). Tests: CoachCardPanel + full web 628 + Go coach green.

**Whole-branch review (G/H/I/K wave): substantially clean, no Critical/High.** Fixed: MEDIUM (SnippetsPane focus loss — keep stable client ids, don't swap to server ids on save); LOW mark double-fire (stopPropagation); LOW sentence `."` under-split (allow a trailing closer as a boundary); LOW sidebar drag-off-screen (viewport clamp).
