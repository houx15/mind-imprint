...
export function ShowcaseLinkEditor({items,onChange}:{items:ShowcaseCustomWork[];onChange:(items:ShowcaseCustomWork[])=>void}) {
  ...
  return <section className="showcase-field-group"><h3>外部作品链接</h3><p className="showcase-mode-help">添加自己制作的网站、作品集或公开项目链接，再在下方勾选展示。</p>
    {items.map(item=><div className="showcase-link-editor" key={item.id}>
    ...
    <button type="button" className="showcase-add-link" onClick={()=>{if(items.length>=100){setError('最多添加 100 个作品链接');return;}onChange([...items,{id:crypto.randomUUID(),title:'',summary:'',url:''}]);}}>添加作品链接</button>
  </section>;
}
