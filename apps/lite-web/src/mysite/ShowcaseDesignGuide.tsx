...
export const GUIDE_STEPS: {id:ShowcaseGuideStage;title:string;message:string;next?:string}[] = [
  ...
  {id:"works",title:"作品",message:"你公开过的阅读和写作报告可以加入作品集。请决定展示哪些，以及访客看到的是卡片、列表，还是另一种形式。",next:"加元素"},
...
export function ShowcaseDesignGuide({stage,draft,interestTree,disabled,available,onBusy,onConversation,onApply,onStep,onComponentIdea}:{
  ...
  useEffect(()=>{const el=conversation.current;if(el)el.scrollTop=el.scrollHeight;},[messages.length,stage,pending]);
  ...
  return <section className="showcase-guide" aria-label="主页设计向导">
    ...
    <div ref={conversation} className="showcase-guide-messages" aria-live="polite">
      <div className="showcase-guide-message"><b>印记</b><p>{step.message}</p>{stage==="profile"&&!!interestTree?.keywords.length&&<p className="showcase-guide-tree">你的兴趣树有：{interestTree.keywords.slice(0,8).join("、")}。你想在主页展示其中哪些？</p>}{stage==="profile"&&<p>介绍完成后，也可以上传照片或生成头像。</p>}{stage==="works"&&<p>本页的「作品」步骤可以管理主页展示。报告是否公开，仍可在各自的阅读或写作报告页管理。</p>}</div>
