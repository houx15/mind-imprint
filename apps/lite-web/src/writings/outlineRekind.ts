...
export function rekindOutlineNode(
  ...
  if (at < 0) return { ok: false, why: "这一条已经不在图上了。" };
  const row = rows[at];
  if (!row) return { ok: false, why: "这一条已经不在图上了。" };
  ...
  if (kind === "thesis") {
    ...
    if (other) {
      return {
        ok: false,
        why: `图上已经有中心论点了（「${other.text}」）。一篇只有一个 —— 先把那一条改成别的，或者删掉它。`,
      };
    }
  }
  ...
  if (parents && parents.length > 0) {
    ...
    if (!has) {
      return {
        ok: false,
        why: `「${outlineKindLabel(kind)}」要挂在${parents
  ...
  if (deepest > 2) {
    return {
      ok: false,
      why: `改成「${outlineKindLabel(kind)}」之后，挂在它底下的东西就没地方放了。先把它们挪开。`,
    };
  }
