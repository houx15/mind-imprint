 ...
 * 标「已替换」。
 *
 * 🚨 **2026-09-20 这一节改过一次，下面两段是新的那一版。** 原来写的是
 * 「折叠 ≠ 关死，她点一下就能回去答」。产品负责人报的第 1 条推翻了它：
 * 「对于多张卡片没有进行卡片管理……旧卡标记已替换并停止接收答案。」屏幕上同时
 * 几张能答的卡，她答哪一张、印记 接哪一张对不上。所以点开之后**只剩题目**：
 * 她看得见 印记 问过什么，答案只收当前那一张。
 *
 * 🚨 **收起来的那张不是置灰的一排待办。** 一排死掉的灰色 UI 读起来是「这是你
 * 没做完的所有事」，计分板的情绪从后门溜进来了。「已替换」说的是这一题换了个
 * 问法，不是她欠下的一笔；展开之后控件**整个撤掉**而不是 disabled——一排点不动
 * 的按钮读起来是「坏了」。
 *
 * `stale` 由 `ReadingCoachPanel` 按「它是不是那张收答案的卡」算：收答案的那张 =
 * 最后到达的那张且她还没答，唯一的例外是服务端明说的一次「回到原题」
 * （辅助题答完、这一步还没走完，见 `coachMessagePayload.Restored`）。
 * 不按「它是不是最新的那张未答卡片」算——后者会让一张早就折起来的旧卡在她答掉
 * 当前这张的瞬间**自己弹开**，下一条回复到达时又折回去
 * （`05-turn1-card-AFTER-tap.png` 拍到的那一下抖动）。
 *
 * ## 求助：一颗按钮，三次封顶，提示长在卡片里
 *
 * 「给点提示」最多三次（`COACH_HINT_CAP`，服务端 `coachHintCap` 有同一道闸），
 * 给满置灰；「示范一下」撤掉。提示（`hints`）显示在这张卡片里而不是对话流里 ——
 * 摆在对话里的话，一条回复把屏幕往下推一截，题目和她写了一半的草稿就被挤出
 * 视野了。三次之后一道开放题可能被一张 `assist` 选择题替掉，她答完回到原题：
 * 回到的是**原来那条消息上那张卡**，组件从头到尾没被卸载，所以草稿还在。
 */
