import { completedSteps, executionStatus, executionLabel, type StepProgressInput } from "./stepProgress";
import "../reports/report-visuals.css";

export function ProjectProgressVisual({ steps, showSteps = true, total = steps.length, done = completedSteps(steps) }: {
  steps: (StepProgressInput & { title: string })[];
  total?: number;
  showSteps?: boolean;
  done?: number;
}) {
  ...
  return <section className="project-progress-visual" aria-label="项目完成进度">
    ...
    {showSteps && steps.length > 0 && <ol className="project-progress-path">{steps.map((step, i) => <li key={i} data-status={executionStatus(step)} title={step.title}><span aria-hidden="true">{executionStatus(step) === "done" ? "✓" : (step.status === "skipped" || step.status === "cancelled") ? "−" : String(i + 1).padStart(2, "0")}</span><div><strong>{step.title}</strong><small>{executionLabel(step)}</small></div></li>)}</ol>}
  </section>;
}
