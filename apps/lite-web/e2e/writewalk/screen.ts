...
export function screenKey(a: WriteAffordances): string {
  return [
    ...
    // 🚨 **她刚写进框里的字，也是「屏幕变了」。**
    //
    // 这一串原来不含输入框的内容。于是她在成稿里把整篇重写一遍之后，
    // key 一个字符都没动 —— 走查判定「屏幕没变化」，把
    //「上一步之后屏幕没有变化」这句话**塞进她的上下文**，连着五步之后
    // 还记一次「老师介入」。
    //
    // 2026-09-12 第十六轮：英文那个学生因此被记了 8 次介入（另一个只有 1 次），
    // 而且她照着那句话得出了产品的结论：「之前明明改了第四段但字数还是370没变，
    // 提示也没消，搞不懂是不是没保存上」。她的字好好地在框里。
    //
    // 走查那只眼睛第四次污染观察本身了（见
    // [[observation-tool-is-the-bug-2026-09-12]]）：前三次是把她的字**藏起来**，
    // 这一次是**告诉她一件没发生的事**，更糟 —— 她会照着它去解释产品。
    //
    // 🚨 **这里要的是那些字本身，不是它们有多少个。**
    //
    // 上面那一次我只把 `.length` 放进了 key，于是同一个毛病第六次复发，而且
    // 是被陪练自己教出来的：印记跟她说「先挪过去试试」——**挪一句话，字数一个
    // 都不变**。第二十八轮她照着做，中文那一路第 31 到 40 步正文一直是 799 字，
    // 内容一直在动，key 一个字符没动。于是连着十步被告知「屏幕没有变化」，
    // 光这一段就记了 6 次老师介入（那一路总共 6 次），clarity 也被拖在 4。
    //
    // 判据只能是逐字的：改了一个字是变化，把两句话对调也是变化。
    a.fields.map((f) => f.value).join(" "),
  ].join("|");
}
