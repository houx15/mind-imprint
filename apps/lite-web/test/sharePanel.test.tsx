...
describe("SharePanel", () => {
  ...
  it("shows the link and a QR image once shared, built from the browser's own origin — never the server's url", async () => {
    ...
    // 这件事在这里是能断的。
    expect(shareReport).toHaveBeenCalledWith("reading", "atom-1", { includeTranscript: false, includeToolkit: false });
  });
  ...
  it("maps kind='writing' onto the writing atom id when sharing/unsharing", async () => {
    ...
    expect(shareReport).toHaveBeenCalledWith("writing", "atom-9", { includeTranscript: false, includeToolkit: false });
