  ...
  return (
    <div className="flex justify-end">
      <button
        ...
        className="max-w-[88%] cursor-pointer rounded-mk-lg border border-white/30 bg-mk-primary px-3.5 py-2.5 text-left text-white transition hover:brightness-105"
        title="点开看你填的这张卡"
      >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-mk-ink/30 px-6" role="dialog" aria-modal="true" onClick={onClose}>
      <div className="flex max-h-[86%] w-[560px] flex-col overflow-hidden rounded-mk-lg border border-mk-border bg-mk-surface shadow-[0_20px_60px_rgba(28,35,51,0.25)]" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between border-b border-mk-border px-6 py-4">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-[10.5px] font-bold uppercase tracking-wide text-mk-primary">工具卡 · 你填的记录</span>
              <span className="rounded-full bg-mk-primary-tint px-2 py-0.5 text-[10.5px] font-bold text-mk-primary">{spec.category}</span>
            </div>
            <h3 className="mt-1 font-sans text-[17px] font-bold text-mk-ink">{spec.name}</h3>
            <p className="mt-0.5 text-[11.5px] text-mk-muted-2">这是你当时填的内容——只读，改不动（这是你的思维记录）。</p>
          </div>
          <button type="button" onClick={onClose} aria-label="关闭" className="text-[20px] leading-none text-mk-muted-2 hover:text-mk-ink">×</button>
        </div>
        <div className="min-h-0 flex-1 overflow-y-auto bg-mk-bg/40 px-6 py-5">
        ...
        <div className="flex justify-end border-t border-mk-border px-6 py-3.5">
          <button type="button" onClick={onClose} className="rounded-mk bg-mk-primary px-4 py-2 text-[13px] font-bold text-white hover:bg-mk-primary-hover">
            知道了
          </button>
        </div>
      </div>
