...
export async function getCourseAssessment(courseId: string): Promise<DualAxisReport | null> {
  const raw = await apiFetch<unknown>(`/api/v1/courses/${courseId}/session/assessment`);
  if (raw == null) return null;
  return DualAxisReport.parse(raw);
}

export async function generateCourseAssessment(courseId: string): Promise<DualAxisReport> {
