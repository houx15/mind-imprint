...
// Mode-independent presentation shared by the project-mode (ParentReport.tsx)
// and stage-mode parent reports: control bar + cover + 这份报告怎么读 + 两条轴
// + 判断怎么来 + {children} (the mode-specific middle) + 生成 button + 下一步
// (advice prop) + 平台接下来会做 + 名词解释 + footer. Moved verbatim from
// ParentReport.tsx so E1's rendered text/queries stay byte-identical.
    ...
    <>
      ...
      <div style={{ maxWidth: 780, margin: "0 auto", padding: "26px 30px 80px" }}>
        ...
        <div style={{ border: "1px solid #E4E7EF", borderRadius: 16, padding: "26px 28px", background: "linear-gradient(180deg,#F7F8FC 0%,#FFFFFF 70%)" }}>
          ...
          {cover.warmLine ? (
            <div style={{ marginTop: 10, fontSize: 14, color: "#5A6377", lineHeight: 1.7 }}>{cover.warmLine}</div>
          ) : null}
