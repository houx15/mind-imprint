...
--    created_at is set explicitly (not the column default) — student_evaluation
--    orders by evaluations.created_at, and an "older" report that defaulted to
--    now() would tie with (or beat) the newer one.
