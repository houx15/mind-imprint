...
- genre：只取 argument / report / narrative / explain / poem / classical / prose。argument 以说服读者接受观点为主；report 报道事件与各方说法；narrative 讲述事件或人物经历；explain 解释概念、原理或过程；poem 是诗、词、曲；classical 是用文言写成的文章；prose 是散文。根据主要写作目的判断，报道、记叙和说明不套用议论文的论证分析。
  - **先看语体和形式，再看写的是什么。** 一篇用文言写的故事是 classical，不是 narrative；一首写景的诗是 poem，不是 narrative。诗、词、曲按分行的韵文形式判断；文言文按语体判断（之、乎、者、也、焉、矣，以及单音节词为主）。
  - 一篇现代白话文即使引用了古诗或文言句子，仍按它自己的体裁判断。
  - **narrative 与 prose 的分界看有没有一件贯穿的事。** 全文由一件事推进、有起因经过结果、人物在这件事里发生变化，是 narrative；没有一件贯穿的事，而是若干片段靠一条线索（一样东西、一处景、一种情感、一条路线）串起来，落点在情感或体会上，是 prose。
  - 写人记事的散文按 narrative 处理：它以叙述为主，读它要把握的同样是情节线索和人物形象。prose 留给写景抒情和写物言志这两类。
  - 两者都像时选 narrative。散文那套读法问的是线索、景物和情感，拿它去读一篇有完整事件的文章，会把那件事的来龙去脉整个跳过。
