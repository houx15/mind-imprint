...
//	网课 / 在线学习
//
// 这张表是**唯一的一道拦截**：没有运行时校验，Pattern 也不带 Topic 字段，
// 屏幕上那句「例句：」后面不会告诉她这句讲的是别的题目。一条例句撞上她正在
// 写的那篇，她照着抄下去就是 AI 替她写了正文（铁律①）。挡住这件事的只有
// 写这条例句的人读没读过这张表。
//
// 🚨 例句还必须是**真的**。这个字段被两条相反的要求夹着：它要短、要像一句
// 写好的话，而写一句漂亮的断言比写一句站得住的断言容易。2026-09-23 之前
// analysis_induce 两条句式**各翻过一次车** —— 雁群南下那条，以及
// 「高原上长大的人……皆来自血液里偏高的红细胞数量」（那是安第斯人的适应方式；
// 藏族高原人群的血红蛋白接近平原水平，红细胞偏高在他们那里是高原红细胞增多症）。
// 教「检查你的概括在每个例子上都成立」的那条方法，摆出来的概括自己不成立。
// 写完一条例句，把它当成一句待查的事实再读一遍。
type Pattern struct {
