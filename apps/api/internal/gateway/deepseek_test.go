...
func TestDeepSeekThinkingByTier(t *testing.T) {
	// Chaperone disables thinking (speed); the mechanical funnel transitions live
	// server-side (reconcileStudioFunnel) so a non-reasoning coach still can't
	// strand the project. Flagship + empty tier keep thinking on for depth.
