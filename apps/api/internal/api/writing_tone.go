...
const writingHostileToneNudge = `刚才那一份里有一句写成了对她的判决（出现了「%s」）。

判断本身是对的，不要改判断。要改的是它的写法：
**说读者在这儿会怎么读，再说她下一步做什么**，不要替她的努力下结论。
...
重新输出一次完整的 JSON：判断、quote、symptom、action 全部保留，只把话换个说法。`
