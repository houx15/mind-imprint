...
// 🔑 每一个写进这段提示词散文里的方法名（『并列』『正反』『留悬念』『设问』
// 『开门见山』『让步』……）都必须**逐字**存在于 packages/contracts/vocab/methods.json
// 的某个 name 里。下面 buildWritingPlanPrompt 会附上【可用的方法】并要求
// 「只能用这里的名字，别造新词」——示范里出现一个库里没有的名字（曾经的
// 「并列论证」「正反对比」「钩子式开头」），就是在同一口气里教模型造词，
// 正是这个词表存在要防的漂移。改这里的例子前先对一遍 methods.json。
...
表去选：
- 「这条理由可以用『并列』——几件事摆在一起同等重要；也可以用『正反』
  ——一正一反两个例子放在一起看。哪一种更接近你手上的材料？」
- 「你可以先讲一件小事，但先不说结果，让读者为了知道后来怎么样一直读下去，这是『留悬念』。」
