...
- 这一轮只给选项，不要再追问。`
	...
	case helpShow:
		// 🚨 这一档的措辞跟着**库里有没有**走，不是固定的一句。
		//
		// 2026-09-23 实测：12 格（lang × genre × 位置）里有 5 格一条句式都
		// 没有 —— 中文侧只有议论文正文段有（三条分析法），开头、结尾、
		// 以及整个中文记叙文都是空的。而这段话原来是无条件写着「本轮提供
		// 一句带空格的通用句式」，下面的列表才是有条件的。
		//
		// 于是那 5 格里，模型被要求交出一条它没有来源的句式 —— 只能现造。
		// 现造出来的多半是议论文形状的（上面例子里那句「因为……，所以……」
		// 就摆在眼前），落到一篇记叙文的结尾上，正是文体这条轴要消灭的东西。
		//
		// 没有句式的时候不降级成「随便给一句」，而是换成这一档仍然做得到、
		// 又不需要库的那件事：指着原文那一处，把要表达的关系说清楚。
		// 梯子不塌（仍然比 helpOffer 具体），也不凭空造词。
...
先核对当前文字是否仍需这项修改；确有需要时，指出原文里具体的那一处，
说明这里要表达的两部分内容之间是什么关系，以及学生还缺其中的哪一部分。
说明关系与位置，不提供可直接复制的正文。`
		}
...
先核对当前文字是否仍需这项修改；确有需要时，从下面这几条里挑一条句式给学生，由学生填写内容。
