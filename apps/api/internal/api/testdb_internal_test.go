...
// testcontainer and replayed ~106 migrations into it. With 381 call sites in
// this package that cost ~2.7s each and made `go test ./internal/api/` a
// ~17-minute wait.
//
...
// and no call site had to be touched.
//
// The helper lives in package `api` (not `api_test`) so that BOTH test
// packages compiled into this binary can share the single container: the
// external `api_test` package dot-imports `mindimprint/api/internal/api`, and
// the test-augmented package it links against includes this file's exported
// NewTestDB.
...
func startSharedPostgres() {
	...
	admin, err := store.NewPool(ctx, dsn)
	if err != nil {
	...
	tmplPool, err := store.NewPool(ctx, tmplDSN)
	if err != nil {
		sharedPGErr = fmt.Errorf("template pool: %w", err)
		return
	}
	migErr := store.RunMigrations(ctx, tmplPool)
	tmplPool.Close()
	if migErr != nil {
...
//
// Exported so the external api_test package (which dot-imports this one) can
// share the same container. Test-only.
func NewTestDB(t *testing.T) *pgxpool.Pool {
	...
	cloneMu.Lock()
	_, err := sharedAdmin.Exec(ctx, `CREATE DATABASE `+name+` TEMPLATE `+templateDBName)
	cloneMu.Unlock()
	if err != nil {
