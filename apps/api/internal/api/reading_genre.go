...
- 标题和正文要对得上：标题说的那件事，正文里哪几句撑着它。
- 系统说明里「作者的主张」「论证」那些说法在这篇上不适用：这里没有作者要她接受的看法。`,
...
- 系统说明里「作者的主张」「论证」那些说法在这篇上换成「说明对象」「原理」。`,
...
- 她给出一种解释时，对比这种解释和原文细节的关系；几种解释都站得住，就说出来有几种。
- 系统说明里「作者的主张」「证据」那些说法在这篇上不适用。`,
}
...
// PRD 第 3 节：「提供『给点提示／示范一下／我自己试试』入口，卡片操作期间仍可
// 向 AI 求助。」前端在敞开的卡片底下摆这两颗按钮，按下去就是她说了这句话。
const (
	coachAskHint = "给点提示"
	coachAskDemo = "示范一下"
)

// helpRequestSection —— 她按了求助按钮的那一轮，多给模型一句怎么接。
//
// 「给点提示」系统说明里那道梯子已经管了（一次给一级）。「示范一下」不一样：
// 梯子里「给我看范例」是第 5 级 —— 整个答案。PRD 要的是**示范一个节点**，
// 剩下的她自己来。所以这一轮单独说清楚。
func helpRequestSection(studentText string) string {
	switch strings.TrimSpace(studentText) {
	case coachAskHint:
		return "\n【她按了「给点提示」】\n按「她卡住的时候」那道梯子，只给**下一级**提示，不给答案。" +
			"她屏幕上那张卡片还开着，不要再发新卡片，advance 留空。\n"
	case coachAskDemo:
		return "\n【她按了「示范一下」】\n拿她屏幕上那张卡片（或那块板）里的**一条**，当面做一遍：" +
			"说出你会怎么处理它，依据是原文里的哪几个字。**只做这一条**，其余的留给她自己做，" +
			"并请她接着做下一条。不要把整张卡做完，不要再发新卡片，advance 留空。\n"
	}
	return ""
}
