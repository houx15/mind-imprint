...
func buildGenreCoachSection(genre string) string {
	...
	if genreHasOrderBoard(genre) {
		b.WriteString("\n**这篇还能用第六种卡片：排序板。**\n" +
			...
			"几件事摆在板上，学生按**发生的先后**排好。options 给 3 到 5 条，每一条都是文章里" +
			"写着一件事的那一句，**逐字抄**（系统会核对）；可以来自同一段。" +
			"板上按原文顺序摆（那就是讲述顺序），你给的顺序不会被用上，所以无需打乱。\n" +
			"什么时候用：清单走到「排出事件顺序 / 排出事件时间线」那一步的时候，用它把这一步交给学生。" +
			"学生排完之后，先回应学生的排序，并说明原文中的先后依据；若讲述顺序与发生顺序不同，再解释其差异与作用，然后推进。" +
			"给板的那一轮不要把正确的先后说出来。\n")
	}
	return b.String()
}
