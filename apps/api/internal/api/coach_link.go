...
// coach_link.go — the link-in-coach → reading bridge. A URL dropped into a coach
// turn is otherwise inert chat text: it is stored verbatim in chat_message but
// never becomes a reference and never points the student at the Reading Room. We
// detect the FIRST new URL server-side (free — a string scan + one library read,
// no model spend) and attach a linkOffer to the coach reply. Detection is
// PHASE-AGNOSTIC by design: whether the student pastes a link while discussing
// the topic, writing the proposal, or drafting the essay, the offer fires — the
// student's tap on the chip is what promotes the link to a reference or opens the
// reading room (铁律: triggering is automatic, opening is confirmed). No prompt
// change: detection is post-hoc, the coach model is untouched.

// urlPattern matches an http(s) URL. Liberal on the body; the trailing-cut below
// strips sentence punctuation a student appends around a pasted link
...
// )]} are deliberately NOT here — they are handled by the balanced trim below so
// a URL whose path legitimately ends in a paren (Wikipedia disambiguation links)
...
// detectLinkOffer returns a linkOffer for the first URL in userInput that is NOT
// already a reference in the project's library, or nil. Dedup is by normalized-
// URL match against ListReferences — an already-added link never re-offers. Any
// query error degrades to nil: a missing offer never fails the turn.
