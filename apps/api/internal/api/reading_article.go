...
var readingArticleTools = []readingBlockTool{
	...
	{
		// 🚨 2026-09-24 起**只管记叙文**。散文分出去自己一件（prose_map）。
		//
		// 原来两种共用，理由是「来源库里散文的依据薄」。但产品负责人给的那份
		// 《记叙文和散文》how-to 把轴说清楚了：「记叙文侧重事件和人物，
		// 散文侧重线索、景物或物象和情感」。共用一套 label 闭表
		// （起因/经过/转折/结果）等于逼一篇《荷塘月色》去交出一件事的来龙去脉，
		// 而它根本没有一件贯穿的事 —— 拿记叙文的「一波三折」去读《春》
		// 会判成「没有结构」。
		Shape: "article", ID: "narrative_map", Label: "事件与人物", Lang: "zh",
		Scope: "article", Genres: []string{genreNarrative},
	...
	{
		// 🚨 2026-09-24 新增。散文自己的那一张，轴是**线索**而不是事件。
		//
		// label 闭表照那份 how-to 的散文考题表（线索与思路、形象/物象、
		// 景物描写作用、表现手法、结构作用），spine 的取值就是它列的线索种类
		// ——「线索可以是人、物、情、理、时、空等」。
		//
		// 🚨 how-to 里那条提醒进了说明：「线索和思路要分开问，学生常把两者
		// 混在一起」。思路回答先写什么后写什么，线索回答围绕什么展开。
		Shape: "article", ID: "prose_map", Label: "线索与情感", Lang: "zh",
		Scope: "article", Genres: []string{genreProse},
		Class: gateway.ClassCompose, MaxRunes: 560,
		Instruction: "把全文拆成 3 到 6 块，做成一张线索与情感图。\n" +
			"每一块的 label 只能取：开篇定调 / 写景 / 写物 / 叙事 / 抒情 / 议论 / 首尾呼应 / 收束。\n" +
			"**spine 写的是线索，不是思路。** 线索回答「全文围绕什么展开」，" +
			"只能取：以人为线索 / 以物为线索 / 以情为线索 / 以理为线索 / 以时间为线索 / 以空间为线索，" +
			"并接一句说它出现在哪几处 —— 一条真的线索会在文中反复出现。\n" +
			"**必须有一块是抒情或议论**：散文的「神」就在那几句里。" +
			"通篇没有直接抒情的句子时，指出哪一处景写得最重，并说它替作者说了什么。\n" +
			"takeaway 写这一篇借这些景物要表达的情感或道理，要具体到「对什么的情感、因为什么」，" +
			"不要只写「表达了作者的热爱」这种放到任何一篇上都成立的话。",
	},
	{
