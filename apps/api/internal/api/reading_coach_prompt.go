...
func readingCurrentStepInstruction(tasks []sqlc.ReadingTask, studentTexts ...string) string {
	...
	case string(taskShape):
		// 🚨 2026-09-23 产品负责人第 5 条：「when reading a 记叙文, it always
		// focuses on very detailed things and ignores the general structure,
		// the writing format etc.」
		//
		// 这一步是整篇那一层唯一的位置，所以它的规矩里最要紧的一句是
		// **不要回到某一句上**。清单上别的每一步都在句子或段落那一层，
		// 少了这句话，这一步会立刻退回去变成又一次精读。
		//
		// 三样东西写成「挑一样谈」而不是三问：一轮一问（铁律③）。
		rule = "本步谈的是整篇的安排，不回到某一个句子的写法上。" +
			"从这三样里挑一样和学生谈：叙述的顺序（按事情发生的先后讲下来，还是先写了后面的事再回头补）、" +
			"详略（哪一段写得最细，哪几段几句话带过）、" +
			"全篇的线索（反复出现的那样东西、那个人、那句话）。" +
			"先说出你在文章里看到的那一处安排（指出是第几段到第几段），再请学生说明作者这样安排的作用。" +
			"学生说出了自己的看法就给 done —— 不要求它和你的说法一致，也不要求学生三样都谈。" +
			"学生说不上来时，把同一处安排换一种问法：这几段如果调换顺序，读起来会差在哪里。"
