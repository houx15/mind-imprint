...
const (
	...
	// 散文。**写作面和阅读面都用，但两边判它的依据不同。**
	//
	// 🚨 写作那边它没有题目词表，只能由她自己说：一篇散文的题目和一篇记叙文的
	// 题目长得一模一样（《背影》《秋天的怀念》都可以是任何一种），从字面上
	// 分不出来 —— 这正是 2026-09-23 那个文体选择器存在的理由之一。
	//
	// 🚨 阅读那边判得出来，因为**正文在手里**：有没有一件贯穿的事，是靠事件
	// 推进还是靠一条线索串起若干片段。2026-09-24 产品负责人确认阅读面要收它
	//（reading-suggestion.md 里「散文、诗歌……先列一下放着」那一句是旧的，
	// 她的原话：「this is very old... and we are about to do them now.」）。
	// 判据写在 ReadingPlanSystem 里，边界照她给的那份《记叙文和散文》how-to：
	// 「记叙文侧重事件和人物，散文侧重线索、景物或物象和情感」。
	genreProse = "prose" // 散文：几个片段，一条线索，一点体会

	// 读后续写。**只在写作面**，阅读那张闭表不收它。
	//
	// 🚨 它不是第五种文体，这一点决定了它长什么样：它从**一篇给定的前文**和
	// 两个印好的段首句出发，写约 150 词的两段。所以它同时用到阅读面的东西
	// （读懂前文的人物、情节线、伏笔）和写作面的东西（两段各自怎么搭）。
	// 放进文体这条轴，是因为文体轴在这个仓库里真正的含义是「这一篇按什么在
	// 教」，而这个问题它答得出来；另起一条轴要把十二个取文体的调用点都改一遍。
	//
