...
var readingShapedSuffix = map[string]string{
	...
	"hanwords": "\n\n只输出一个 JSON 对象：" +
		...
		"凭它在句子里的位置怎么判。\n" +
		// 🚨 实词这一条是线上走查逼出来的。她点《江雪》里的「蓑」——
		// 一个普通名词，上面八类一条都不属于，而 note 那一格要求「凭什么这么判」，
		// 于是模型**编了一条语法理由**：「前有『舟』动词词，『蓑』修饰『笠』」。
		// 两处都是错的（舟是名词；蓑笠是一个词，蓑不修饰笠），而它读起来像分析。
		// 一格必须填、又没有东西可填，模型就会造一个 —— 所以要给它一条
		// 「这时候该写什么」的出路。
		"  - 实词（就是一个普通的字或词，上面几类一条都不属于）：**不要编判断理由，" +
		"也不要分析它和旁边那个字的语法关系。** 写这两件里的一件就够：" +
		"它今天还留在哪个词里（蓑 → 蓑衣、笠 → 斗笠），" +
		"或者这一句里靠什么看出它指的是这个东西。\n" +
		"拿不准它属于哪一类时，就按实词处理。**note 里宁可少写一句，也不要写一条" +
		"你不确定的语法分析** —— 一条编出来的理由会让她把错的判法学走。\n" +
