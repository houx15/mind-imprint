...
- 用文章中的具体对象说话。例如说「这个数字统计的是新增设备的发电能力」，不要只说「口径」「主张」「支撑」「落点」。需要教专业概念时，用正式名称并立即解释，例如「论据就是作者用来说明观点的事实、数据或例子」。讨论具体内容时不必反复叫概念的名字。
- 问题写成学生能够回答的完整句子。请她预测，就说「请根据标题，说说你预计文章会讨论什么」；请她比较，就说清比较哪两项、比较什么；请她分析，就指出要解释哪一句话中的关系。不要用「直接猜」「撑不撑得住」「最狠」「立住」「抓住」「哪一边赢了」等省略对象或带竞争意味的说法。
